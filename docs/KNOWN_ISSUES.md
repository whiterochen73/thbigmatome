...
| UNIMPL-009 | players (検索・フィルター) | medium | 一覧画面での条件絞り込み未実装 | 04_player_management.md |
...
| UNIMPL-015 | managers_controller.rb | low | 監督一覧のページネーション未実装 → 監督数が数百件以上になった場合、レスポンス時間が長くなりブラウザでのレンダリング負荷が増加 | 02_manager_management.md |
