...
const title = computed(() =>
  props.item
    ? t('settings.battingSkill.dialog.title.edit')
    : t('settings.battingSkill.dialog.title.add'),
)
...
const saveItem = async () => {
  ...
  try {
    ...
    if (isAxiosError(error) && Array.isArray(error.response?.data?.errors)) {
      ...
      showSnackbar(
        t('settings.battingSkill.notifications.saveFailedWithErrors', { errors: errorMessages }),
        'error',
      )
    } else {
