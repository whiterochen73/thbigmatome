...
<template>
  <v-container>
    ...
    <v-toolbar color="primary">
      <template #prepend>
        <h1 class="text-h5">{{ t('playerAbsenceHistory.title') }}</h1>
      </template>
      <v-btn class="mx-2" variant="outlined" @click="editAbsence(null)">
      ...
      <v-card-text>
        <v-data-table :headers="headers" :items="playerAbsences" item-key="id" class="elevation-1">
          ...
          <template v-slot:item.actions="{ item }">
            <v-icon small class="mr-2" @click="editAbsence(item)"> mdi-pencil </v-icon>
            <v-icon small @click="deleteAbsence(item)"> mdi-delete </v-icon>
          </template>
        </v-data-table>
      </v-card-text>
    </v-card>
