...
シーズン開始時に、各チームは1選手を「聖域」として指定できる。

| ルール | 値 |
|--------|-----|
| 1チームあたりの指定数 | 1人 |
| 野手特例と投手特例の同時使用 | **不可**（どちらか一方のみ） |
| 条件喪失時の扱い | 無効化（別選手への再指定不可） |

---

### 3.7.1 野手特例（怪我の回避）

**有効条件**: フルイニング出場継続中のみ有効
...
> チームはこの選手を「聖域として守る義務」を負う。

**失効条件**:
- 退場等の**不可抗力**でフルイニング出場が途切れた場合 → 失効（やむを得ない場合）
- 自分から下げた（スタメン外し・交代指示）場合 → 失効（かつ再指定不可）
...
- → 第2章: チーム構成ルール（ロスター制約・コスト制度・借金特例）
