-- ============================================================================
-- v0.3.1 本番カード登録SQL（A5:SQL Mk-2互換版）
-- 対象: PM2026 未登録カード4件
-- 実行環境: 本番 thbigmatome_production DB
-- 実行方法: A5M2 or psql で直接実行
--   docker exec projects-db-1 psql -U thbigmatome -d thbigmatome_production -f v0.3.1_card_registration.sql
...
INSERT INTO player_cards (
  ...
  (SELECT id FROM card_sets WHERE name = 'PM2026'),
  (SELECT id FROM players WHERE REPLACE(REPLACE(name, ' ', ''), '　', '') = '西行寺幽々子' AND series = 'touhou' LIMIT 1),
  'batter', '楼閣',
...
INSERT INTO player_cards (
  ...
  (SELECT id FROM card_sets WHERE name = 'PM2026'),
  (SELECT id FROM players WHERE REPLACE(REPLACE(name, ' ', ''), '　', '') = '八雲藍' AND series = 'touhou' LIMIT 1),
  'batter', 'WBC2',
  false, false, false, false, false,
...
INSERT INTO player_cards (
  ...
  (SELECT id FROM card_sets WHERE name = 'PM2026'),
  (SELECT id FROM players WHERE REPLACE(REPLACE(name, ' ', ''), '　', '') = '花山栄美' LIMIT 1),
  'batter', 'UR',
  false, false, false, false, false,
...
INSERT INTO player_cards (
  ...
  (SELECT id FROM card_sets WHERE name = 'PM2026'),
  (SELECT id FROM players WHERE REPLACE(REPLACE(name, ' ', ''), '　', '') = '小鳥遊柚' LIMIT 1),
  'batter', 'UR',
  false, false, false, false, false,
