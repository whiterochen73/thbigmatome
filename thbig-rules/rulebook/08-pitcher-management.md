...
```
INJURY_CHECK:
  INPUT: trigger_condition (上記1〜4のいずれか), accumulated_innings (リリーフのみ)

  STEP 1: けが特徴レベルの決定
    IF trigger_condition == リリーフ（累積3I以上）:
      累積3I     → けが特徴5（Wikiに具体値の記載なし、4Iと同じLv5として扱う）
      累積4I     → けが特徴5
      累積5I     → けが特徴4
      累積6I     → けが特徴3
      累積7I以上 → けが特徴2
...
INJURY_ABSENCE_RULES:
  ...
  accumulated_innings = (離脱前の値をそのまま引き継ぐ)
  ※ 減衰ルール（§8.3.2）は離脱中の日数分も適用される
```
