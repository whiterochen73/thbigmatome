  ...
  <v-dialog v-model="isOpen" max-width="600px">
    <v-card>
          ...
          <v-col cols="4">
            {{
              calendarDate
                ...
                : t('settings.schedule.detail.selectDate')
            }}
            <v-radio-group v-model="selectedDateType">
              <v-radio
                v-for="(type, i) in dateTypes"
                :key="i"
                :label="type.label"
                :value="type.value"
              >
                ...
                <v-btn
                  :color="
                    dateTypes.find(
                      (type) =>
                        type.value ===
                        scheduleDetails.find((detail) => detail.date === dateToString(item.date))
                          ?.date_type,
                    )?.color || 'transparent'
                  "
                  block
      ...
      <v-card-actions>
        ...
        <v-btn color="accent" variant="flat" @click="save">{{ t('actions.save') }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
const dateTypes = computed(() => [
  ...
  {
    label: t('settings.schedule.dateTypes.interleague_game_day'),
    value: 'interleague_game_day',
    color: 'deep-purple',
  },
  ...
  {
    label: t('settings.schedule.dateTypes.interleague_reserve_day'),
    value: 'interleague_reserve_day',
    color: 'brown',
  },
...
const fetchScheduleDetails = async () => {
  ...
  const response = await axios.get<ScheduleDetail[]>(
    `/schedules/${props.schedule.id}/schedule_details`,
  )
