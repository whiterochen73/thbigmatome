...
<template>
  <v-container>
    ...
    <v-tabs v-model="activeTab" class="mt-2">
      <v-tab value="roster">{{ t('activeRoster.tabs.roster') }}</v-tab>
      <v-tab value="history">{{ t('activeRoster.tabs.history') }}</v-tab>
    </v-tabs>
        ...
        <v-row v-if="nearEndAbsencePlayers.length > 0" class="mt-2">
          <v-col cols="12">
            <v-alert type="info" density="compact" elevation="2">
              ...
              <p
                v-for="player in nearEndAbsencePlayers"
                :key="player.team_membership_id"
                class="mb-0"
              >
                {{ player.player_name }}:
