  ...
  <v-container>
    <v-toolbar color="green">
      <template #prepend>
        ...
        <span
          class="text-h6 mx-4"
          v-if="gameData.score !== null && gameData.opponent_score !== null"
        >
    ...
    <v-table v-if="gameData.scoreboard" density="compact" class="scoreboard-table">
      <thead>
        <tr>
          ...
          <th v-for="(_, index) in gameData.scoreboard.away" :key="index" class="inning-header">
            {{ index + 1 }}
          </th>
    ...
    <v-table v-if="startingMembers.length > 0" class="batting-record-table mt-6">
      ...
      <tbody>
        <tr v-for="player in startingMembers" :key="player.id">
...
const defaultGameData: GameData = {
  ...
  score: null,
  opponent_score: null,
  winning_pitcher_id: null,
  losing_pitcher_id: null,
...
const atBatOptions = computed(() => [
  t('scoreSheet.atBatResults.single'),
  t('scoreSheet.atBatResults.double'),
  t('scoreSheet.atBatResults.triple'),
  t('scoreSheet.atBatResults.homerun'),
  t('scoreSheet.atBatResults.sacrifice_hit'),
  t('scoreSheet.atBatResults.sacrifice_fly'),
  t('scoreSheet.atBatResults.walk'),
  t('scoreSheet.atBatResults.hit_by_pitch'),
  t('scoreSheet.atBatResults.strikeout'),
  t('scoreSheet.atBatResults.double_play'),
  t('scoreSheet.atBatResults.ground_out'),
  t('scoreSheet.atBatResults.fly_out'),
])
...
const initialOpponentLineupForDialog = computed(() => {
  ...
  return opponentStartingMembers.value
    .map((player) => {
...
const homeTeamName = computed(() => {
  ...
  return gameData.value.home_away === 'home'
    ? gameData.value.team_name
    : gameData.value.opponent_team_name
})

const awayTeamName = computed(() => {
  if (!gameData.value) return ''
  return gameData.value.home_away === 'visitor'
    ? gameData.value.team_name
    : gameData.value.opponent_team_name
})
...
const fetchGameData = async () => {
  try {
    ...
    // opponent team
    if (
      gameData.value.opponent_starting_lineup &&
      gameData.value.opponent_starting_lineup.length > 0
    ) {
...
const handleSaveStartingMembers = async (data: {
  homeLineup: StartingMember[]
  opponentLineup: StartingMember[]
}) => {
  // home team
  ...
  startingMembers.value = sortedHomeMembers
    .map((m) => m.player)
  ...
  const sortedOpponentMembers = [...validOpponentMembers].sort(
    (a, b) => a.battingOrder - b.battingOrder,
  )
  opponentStartingMembers.value = sortedOpponentMembers
    .map((m) => m.player)
    .filter((p): p is Player => p !== null)
  ...
  sortedOpponentMembers.forEach((member) => {
    if (member.player && member.position) {
      opponentPositions[member.player.id] = {
        position: member.position,
        order: member.battingOrder,
      }
    }
  })
  ...
  if (homeLineupToSave.length === 0) {
    showSnackbar(t('messages.noMembersToSave'), 'warning')
    return
  }
